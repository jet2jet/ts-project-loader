...
function parseWrappedFsPath(rootPath: string, pathName: string): string | null {
	return (pathName.substring(0, rootPath.length) === rootPath)
		? pathName.substring(rootPath.length - 1)
		: null;
}

function makeWrappedFsPath(rootPath: string, pathName: string): string {
	if (pathName[0] === '/') {
...
class WrappedFsWatcher implements WatchManager {
	...
	constructor(
		...
		const memRootPath = wrappedFs.rootPath;
		...
		files.forEach((file) => {
			const p = parseWrappedFsPath(memRootPath, file);
			if (p === null) {
				origFiles.push(file);
			} else {
				memFiles.push(file);
		...
		dirs.forEach((dir) => {
			const p = parseWrappedFsPath(memRootPath, dir);
			if (p === null) {
				origDirs.push(dir);
			} else {
				memDirs.push(dir);
		...
		missing.forEach((m) => {
			const p = parseWrappedFsPath(memRootPath, m);
			if (p === null) {
				origMissing.push(m);
			} else {
				memMissing.push(m);
	...
	public getFileTimestamps(): Map<string, number> {
		const { rootPath, statMeta, fs } = this.wrappedFs;
		let r = this.files.concat(this.dirs).map((name): [string, number] => {
			const p = parseWrappedFsPath(rootPath, name);
			const stat = getStatMeta(statMeta, fs.pathToArray(p!));
	...
	public onFileAdded(name: string, mtime: number) {
		const wrappedName = makeWrappedFsPath(this.wrappedFs.rootPath, name);
	...
	public onFileRemoved(name: string) {
		// console.log('*** onFileRemoved', makeWrappedFsPath(this.wrappedFs.rootPath, name));
		this._addAggregateChanges(makeWrappedFsPath(this.wrappedFs.rootPath, name));
	}
}

////////////////////////////////////////////////////////////////////////////////

/** Generated WrappedFs count (used for the ID) */
let _wrappedFsCount = 0;

/**
 * Implements ts.System with memory-fs.
 * File name with special prefix (rootPath) will refer to the data in memory-fs.
...
export default class WrappedFs implements ts.System {
	...
	constructor() {
		...
		// make unique root-path to distinguish another WrappedFs
		this.rootPath = `ts-project-fs${++_wrappedFsCount}:/`;

		this.fs = new MemoryFileSystem();
	}

	public isInputFileSystemWrapped(baseFs: webpack.InputFileSystem): boolean {
		if ((baseFs as any)._wrappedFs === this) {
			return true;
		}
		// check multi-wrapped fs
		if ((baseFs as any)._baseFs) {
			return this.isInputFileSystemWrapped((baseFs as any)._baseFs);
		}
		return false;
	}
	...
	public makeInputFileSystem(baseFs: webpack.InputFileSystem, tscBuildConfig: TscBuildConfig): webpack.InputFileSystem {
		...
		return <any>{
			...
			readFile(path: string, callback: (err: Error | undefined | null, contents: Buffer) => void): void {
				const p = parseWrappedFsPath(rootPath, path);
				if (p === null) {
			...
			readdir(path: string, callback: (err: Error | null, result?: any) => void): void {
				const p = parseWrappedFsPath(rootPath, path);
				if (p === null) {
					return baseFs.readdir(path, callback);
				}
				let result: string[];
				try {
			...
			readdirSync(path: string): string[] {
				const p = parseWrappedFsPath(rootPath, path);
				if (p === null) {
					return baseFs.readdirSync(path);
				}
			...
			createReadStream(
				...
				const p = parseWrappedFsPath(rootPath, path);
				if (p === null) {
					return baseFs.createReadStream(path, options);
				}
			...
			exists(path: string, callback: (isExist: boolean) => void): void {
				const p = parseWrappedFsPath(rootPath, path);
				if (p === null) {
					return baseFs.exists(path, callback);
				}
			...
			join(path: string, request: string): string {
				...
				if (p === null) {
					if (baseFs.join) {
						return baseFs.join(path, request);
					} else {
						return nodePath.join(path, request);
					}
				}
				return self.joinPath(path, request);
			},
			pathToArray(path: string): string[] {
				return baseFs.pathToArray ? baseFs.pathToArray(path) :
					path.split(/\\\//g);
			},
			normalize(path: string): string {
				const p = parseWrappedFsPath(rootPath, path);
				if (p === null) {
					return baseFs.normalize ? baseFs.normalize(path) : nodePath.normalize(path);
				}
				return makeWrappedFsPath(rootPath, normalizeWrappedFsPath(self.fs, p));
			}
		};
	}

	public isWrappedFsPath(pathName: string): boolean {
		return parseWrappedFsPath(this.rootPath, pathName) !== null;
	}

	public joinPath(path1: string, path2: string) {
		const p1 = parseWrappedFsPath(this.rootPath, path1);
		if (p1 !== null) {
	...
	public relativePath(basePath: string, targetPath: string) {
		...
		const p1 = parseWrappedFsPath(rootPath, basePath);
		const p2 = parseWrappedFsPath(rootPath, targetPath);
		if (p1 !== null && p2 !== null) {
	...
	public onFileDeleted(files: string[]) {
		const { rootPath, statMeta, fs, watchers } = this;
		files.forEach((file) => {
			const p = parseWrappedFsPath(rootPath, file);
			if (p !== null) {
				const a = fs.pathToArray(p);
				fs.unlinkSync(p);
				removeStatMeta(statMeta, a, watchers);
			}
		});
	}
	...
	public readFile(path: string, encoding?: string): string | undefined {
		const p = parseWrappedFsPath(this.rootPath, path);
		if (p === null) {
	...
	public writeFile(path: string, data: string, writeByteOrderMark?: boolean): void {
		// console.log('*** WrappedFs: writeFile:', path);
		const p = parseWrappedFsPath(this.rootPath, path);
		if (p === null) {
		...
		// check if the content is not modified (for watch mode)
		try {
			const oldData = this.fs.readFileSync(p, 'utf8');
			if (typeof oldData === 'string' && oldData.length === data.length && oldData === data) {
				return;
			}
		} catch (_e) { }
	...
	public resolvePath(path: string): string {
		...
		return makeWrappedFsPath(this.rootPath, normalizeWrappedFsPath(this.fs, p));
	}
	...
	public getDirectories(path: string): string[] {
		...
		).map((name) => makeWrappedFsPath(rootPath, name));
	}
